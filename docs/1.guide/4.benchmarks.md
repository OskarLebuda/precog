...
**Predictions in the published run came from the playground's stand-in, not from Jev.** The
latency and token figures are therefore made up; only the shape of the pipeline is real. Run
with `PRECOG_MOCK=0` and a key for real ones.
